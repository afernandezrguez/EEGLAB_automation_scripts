...
if nargin < 1  % No input arguments specified, show GUI
    ...
    if iscell(fileName)
        ...
        indices_ = strfind(files(1).name,'/');
        filePath = extractBetween( files(1).name, 1, indices_( length(indices_) ) );
...
if nargin<1 %GUI
    ...
    % New code: We remove the inputGUI to select always [Code, Type and Begin]
    events = {'StimulusCode'; 'StimulusType'; 'StimulusBegin'};
...
if length(files)>1
    ...
    %Is there a bug here? I think so, it lacks a row indicating the end of
    %each region. Let's fix it:
    
   EEG.event = eeg_insertboundold(EEG.event, EEG.pnts * EEG.trials, fileBorders');    
  %  EEG.event = eeg_insertboundold(EEG.event, EEG.pnts * EEG.trials, fileBorders');
    %end of possible fix
...
%New code to save filePath to be used in the automation for analysis
EEG.loadedfilePath = char(filePath);
% end of new code
